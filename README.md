...
动态黑名单: 起始路径(除了白名单外)位于动态黑名单内且判定为文件且距上次访问时间之差达`TIME_LIMIT * 3`(当前为1分钟),对该路径执行策略是报错返回.目前动态黑名单最大数量设定为10.判定: 30秒内访问次数达到15次将进入动态黑名单.此名单中的记录~~不会主动移除~~在时间之差达标时移除,也会被 新数据 且 新数据在主程序中定义的`hashFunction`函数所得到相同的返回值的情况下 覆写.
...
监控程序似乎有点问题,如: 1️⃣带参启动主程序时,监控程序会启动失败.2️⃣监控程序收到SIGTERM信号后,并不能成功结束主程序.
